...
contract TokenVest is ERC721, Ownable {
    ...
    IERC20 public constant paymentToken = IERC20(0x47f917EE1b0BE0D5fB51d45c0519882875fB3457);
    ...
    function safeMint(address to, uint256 numTokens) public  {
        require(_tokenIdCounter.current() + numTokens <= MAX_SUPPLY, "Token minting would exceed max supply");
        uint256 totalPrice = TOKEN_PRICE * numTokens;
        require(paymentToken.balanceOf(msg.sender) >= totalPrice, "Not enough tokens for purchase");
        require(paymentToken.allowance(msg.sender, address(this)) >= totalPrice, "Contract not allowed to transfer enough tokens");
